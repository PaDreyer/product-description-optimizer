...
class TestExportValid:
    def test_exports_done_products(
    ...
    def test_csv_has_original_plus_extra_columns(
        self, db: Database, tmp_path: Path
    ) -> None:
...
class TestExportIncludeErrors:
    def test_includes_error_products(
        self, db: Database, tmp_path: Path
    ) -> None:
        db.insert_products([
            {
        ...
        db.update_product_status(
            products[0]["id"], "done", optimized_description="optimized"
        )
        db.update_product_status(
            products[1]["id"], "error", error_message="failed"
        )
    ...
    def test_excludes_error_by_default(
        self, db: Database, tmp_path: Path
    ) -> None:
        db.insert_products([
            {
        ...
        products = db.get_all_products()
        db.update_product_status(
            products[0]["id"], "error", error_message="fail"
        )
...
class TestExportEmpty:
    ...
    def test_pipeline_state_set_to_done(
        self, db: Database, tmp_path: Path
    ) -> None:
        _seed_and_optimize(db, 2)
        output = tmp_path / "out.csv"
