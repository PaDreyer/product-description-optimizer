...
from pdo.cli.client import connect, send_command
from pdo.cli.main import cli, version
...
def test_version_command_daemon_not_running(mock_send):
    ...
    assert f"Client version:" in result.output
...
def test_version_command_mismatch(mock_send):
    ...
    mock_send.return_value = Response(
        success=False, 
        error="Version mismatch: CLI client is v0.1.0, but daemon is v0.2.0"
    )
