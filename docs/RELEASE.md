...
- **Linux:** Run `scripts/build-linux-container.sh`. Its Debian 12 image provides Python 3.12, Qt system libraries, and appimagetool. The script installs `requirements-build.txt` in an isolated build environment, runs desktop and pipeline tests, validates the desktop and AppStream metadata, smoke-tests the PyInstaller application, and creates AppImage and Debian packages. Both packages include the AppStream metadata.
- **Windows:** Run `scripts/build-windows.ps1`. It installs `requirements-build.txt` in an isolated build environment, runs desktop and pipeline tests, smoke-tests the PyInstaller application, and creates the installer with Inno Setup.
